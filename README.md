...
### Requisitos Técnicos:
- Utilizar **Node.js** e **Express** para criar a API.
- Utilizar **Prisma** para modelagem de dados, definindo as entidades e seus relacionamentos.
- **Sem Banco de Dados Real**: 
  - Definir os modelos Prisma (`User` e `Document`) e as relações entre eles.
  - Não é necessário configurar ou conectar a um banco de dados real. As operações de CRUD podem ser simuladas usando objetos ou arrays em memória.
...
- Os modelos devem ser definidos no arquivo `schema.prisma`, mas não é necessário executar migrações ou criar um banco de dados real.
...
- As respostas das rotas podem ser simuladas com objetos ou arrays em memória (não é necessário persistir dados em um banco real).
...
### React (Opcional):
- Crie um exemplo básico de código em **React**, demonstrando como faria chamadas para a API criada no teste e como exibiria os dados para o usuário na interface.
- Não é necessário criar um projeto React completo – apenas um arquivo com o código suficiente para entendermos sua abordagem.
